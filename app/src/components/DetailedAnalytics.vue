...
function updateAllCharts() {
    ...
    data.forEach(d => {
        ...
        if (!hourlyStats[hour]) hourlyStats[hour] = { times: [], count: 0 };
    ...
    perfTimelineChart.data = { labels, datasets: [
        { label: 'Avg', data: labels.map(l => hourlyStats[l].times.reduce((s, t) => s + t, 0) / hourlyStats[l].count), borderColor: '#52c5f7', tension: 0.3, pointRadius: 0 },
        { label: 'P95', data: labels.map(l => hourlyStats[l].times.sort((a,b)=>a-b)[Math.floor(hourlyStats[l].times.length * 0.95)]), borderColor: '#ffb74d', tension: 0.3, pointRadius: 0 },
        { label: 'P99', data: labels.map(l => hourlyStats[l].times.sort((a,b)=>a-b)[Math.floor(hourlyStats[l].times.length * 0.99)]), borderColor: '#f2495c', tension: 0.3, pointRadius: 0 },
    ]};
    perfTimelineChart.update();

    // RPS Timeline
    rpsTimelineChart.data = { labels, datasets: [{ label: 'RPS', data: labels.map(l => hourlyStats[l].count / 3600), borderColor: '#73bf69', fill: true, backgroundColor: 'rgba(115, 191, 105, 0.1)', tension: 0.3 }]};
    ...
    // Status & Method
    const statusCounts = data.reduce((acc, d) => { acc[d.status_code] = (acc[d.status_code] || 0) + 1; return acc; }, {});
    statusChart.data = { labels: Object.keys(statusCounts), datasets: [{ data: Object.values(statusCounts), backgroundColor: ['#73bf69', '#fade2a', '#ffb74d', '#f2495c'] }]};
    statusChart.update();
    
    const methodCounts = data.reduce((acc, d) => { acc[d.method] = (acc[d.method] || 0) + 1; return acc; }, {});
    methodChart.data = { labels: Object.keys(methodCounts), datasets: [{ data: Object.values(methodCounts), backgroundColor: ['#52c5f7', '#73bf69', '#ffb74d', '#f2495c', '#b877d9'] }]};
    methodChart.update();

    // Traffic by Hour
    const trafficByHour = new Array(24).fill(0);
    data.forEach(d => trafficByHour[new Date(d.timestamp).getHours()]++);
    ...
    // Endpoint Charts
    const endpointStats = data.reduce((acc, d) => {
    ...
    slowestEndpointsChart.data = { labels: sortedByTime.map(([k]) => k), datasets: [{ label: 'Avg Response (ms)', data: sortedByTime.map(([,v])=>v.totalTime/v.count), backgroundColor: '#ffb74d' }]};
    ...
    errorEndpointsChart.data = { labels: sortedByError.map(([k]) => k), datasets: [{ label: 'Error Rate (%)', data: sortedByError.map(([,v])=>(v.errors/v.count)*100), backgroundColor: '#f2495c' }]};
